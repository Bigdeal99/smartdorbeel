...
namespace Service
{
    public class BlobStorageService
    {
        ...
        public async Task<string> SearchImagesByDateAsync(string searchDate)
        {
            var containerClient = _blobServiceClient.GetBlobContainerClient("iot-10sec-video");
            var images = new List<ImageData>();
            var sasToken = @"sp=racwdli&st=2024-05-31T22:29:05Z&se=2024-07-06T06:29:05Z&sv=2022-11-02&sr=c&sig=y1coXPiJxbwFwE0H9c8ydn0kd715N%2BKE1n1q6H%2BuuwY%3D";

            await foreach (var blobItem in containerClient.GetBlobsAsync())
            {
                var fileName = blobItem.Name;
                if (fileName.StartsWith(searchDate))
                {
                    var blobClient = containerClient.GetBlobClient(blobItem.Name);
                    var uri = $"{blobClient.Uri}?{sasToken}";
                    images.Add(new ImageData { Url = uri.ToString(), FileName = blobItem.Name });
